...
public class IncidentPanel extends ExtensionPanel {
    ...
    public void drawPanel() {
        ...
        for (Accident accident : accidents) {
            ...
            applet.rect(x, y, lineHeight, lineHeight);
            applet.fill(255);
            applet.textAlign(CENTER, CENTER);
            applet.text("" + accident.getIncidentNumber(), x + lineHeight / 2, y + lineHeight / 2);
            applet.textAlign(LEFT, CENTER);
            applet.text(TimeUtils.asDuration(accident.getEarliestTime()),
            ...
            for (int carId : accident.getCars()) {
                ...
                applet.rect(300 + xx, y, lineHeight * 1.5f, lineHeight);
                applet.fill(255);
                applet.textAlign(CENTER, CENTER);
                applet.text("" + extension.getModel().getCar(carId).getCarNumber(),
                        300 + xx + lineHeight * 1.5f / 2, y + lineHeight / 2);
            }
        }
        LookAndFeel.drawScrollBar(applet, accidents.size(), visibleLines, scroll);
    }
